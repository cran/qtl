         ...
         geno2, main)
{
  ...
  if(missing(mark1)) { # no data given
    if(missing(mname1)) stop("Either mname1 or mark1 must be specified.")
    ...
    if(type=="f2" || type=="f2ss") mark1[mark1>3] <- NA
    if(type=="4way") mark1[mark1>4] <- NA

    # if X chr and backcross or intercross, get sex/dir data + revise data
    if(chrtype1 =="X" && (type=="bc" || type=="f2" || type=="f2ss")) {
      sexpgm <- getsex(cross)
      mark1 <- as.numeric(fixXdata(type, "full", sexpgm, geno=as.matrix(mark1)))
      gennames1 <- getgenonames(type, chrtype1, "full", sexpgm)
    }
  }
  else {
    if(length(mark1) != n.ind) 
      stop("Marker 1 data is the wrong length")
    if(missing(mname1)) mname1 <- "Marker 1"
  }

  # Deal with marker 2
  if(!missing(mname2) || !missing(mark2)) { # second marker included
    if(missing(mark2)) {
      # find chromosome containing marker
      o <- sapply(cross$geno,function(a,b)
      ...
      # turn partially informative genotypes into NA's
      if(type=="f2" || type=="f2ss") mark2[mark2>3] <- NA
      if(type=="4way") mark2[mark2>4] <- NA

      # if X chr and backcross or intercross, get sex/dir data + revise data
      if(chrtype2 =="X" && (type=="bc" || type=="f2" || type=="f2ss")) {
        sexpgm <- getsex(cross)
        mark2 <- as.numeric(fixXdata(type, "full", sexpgm, geno=as.matrix(mark2)))
        gennames2 <- getgenonames(type, chrtype2, "full", sexpgm)
      }
    }
    else {
      if(length(mark2) != n.ind) 
        stop("Marker 2 data is the wrong length")
      if(missing(mname2)) mname2 <- "Marker 2"
    }
  }
  else mark2 <- NULL
  ...
  if(!missing(geno1)) {
    ...
    if(!is.null(gennames1)) geno1 <- gennames1
    else if(is.factor(mark1)) {
      geno1 <- levels(mark1)
      mark1 <- as.numeric(mark1)
    }
    else {
      if(type=="bc") geno1 <- c("AA","AB")
      else if(type=="f2" || type=="f2ss") geno1 <- c("AA","AB","BB")
      else if(type=="riself" || type=="risib") geno1 <- c("AA","BB")
      else if(type=="4way") geno1 <- c("AC","BC","AD","BD")

      if(length(unique(mark1)) > length(geno1)) 
        geno1 <- c(geno1,rep("?", length(unique(mark1)) - length(geno1)))
    }
  }
  ...
  if(!is.null(mark2)) {
    if(!missing(geno2)) {
      ...
      if(!is.null(gennames2)) geno2 <- gennames2
      else if(is.factor(mark2)) {
        geno2 <- levels(mark2)
        mark2 <- as.numeric(mark2)
      }
      else {
        if(type=="bc") geno2 <- c("AA","AB")
        else if(type=="f2" || type=="f2ss") geno2 <- c("AA","AB","BB")
        else if(type=="riself" || type=="risib") geno2 <- c("AA","BB")
        else if(type=="4way") geno2 <- c("AC","BC","AD","BD")

        if(length(unique(mark2)) > length(geno2)) 
        geno2 <- c(geno2,rep("?", length(unique(mark2)) - length(geno2)))
      }
    }
  }
  ngen1 <- length(geno1)
  ...
  if(is.null(mark2)) {
    ...
    ses <- tapply(pheno, mark1, function(a)
                  sd(a,na.rm=TRUE)/sqrt(sum(!is.na(a))))
    lo <- means-ses
    hi <- means+ses
    ...
    ses <- tapply(pheno, list(mark1,mark2), function(a)
                  sd(a,na.rm=TRUE)/sqrt(sum(!is.na(a))))
    lo <- means-ses
    hi <- means+ses
    ...
    if(nrow(means) != length(geno1)) {
      ...
      if(nrow(means) < length(geno1))
        ...
        geno1 <- c(geno1,rep("?",nrow(means)-length(geno1)))
      ngen1 <- length(geno1)
    }
  ...
  if(missing(main)) {
    if(is.null(mark2))
      ...
      main <- paste("Interaction plot for", mname1, "and", mname2)
  }
  ...
  if(is.null(mark2)) { # single marker
    # plot the means
    plot(1:ngen1, means, main=main, xlab=mname1,
         ylab=names(cross$pheno)[pheno.col], pch=1, col="black",
         ylim=ylimits, xaxt="n", type="b", xlim=xlimits)
    # confidence limits
    for(i in 1:ngen1) {
      if(!is.na(lo[i]) && !is.na(hi[i]))
        lines(c(i,i), c(lo[i],hi[i]), pch=3, col="black",type="b", lty=3)
    }
    # X-axis ticks
    a <- par("usr")
    ystart <- a[3]
    ...
    for(i in 1:ngen2) {
      if(!is.na(lo[1,i]) && !is.na(hi[1,i]))
        lines(c(i,i), c(lo[1,i],hi[1,i]),
              pch=3, col=int.color[1], type="b", lty=3)
    }
           
    for( j in 2:ngen1 ) { # for the rest of genotypes for Marker 1
      lines(1:ngen2, means[j,], col=int.color[j], pch=1, type="b")
      # confidence limits
      for(i in 1:ngen2) {
      if(!is.na(lo[j,i]) && !is.na(hi[j,i]))
        lines(c(i,i), c(lo[j,i],hi[j,i]),
              pch=3, col=int.color[j], type="b", lty=3)
      }
    }
